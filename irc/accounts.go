...
const (
	...
	keyAccountUsername         = "account.username %s" // client username stored as string
...
func (am *AccountManager) saveUsername(account string, username string) {
	key := fmt.Sprintf(keyAccountUsername, account)
	am.server.store.Update(func(tx *buntdb.Tx) error {
		if username != "" {
			tx.Set(key, username, nil)
		} else {
			tx.Delete(key)
		}
		return nil
...
func (am *AccountManager) loadUsername(account string) (username string) {
	key := fmt.Sprintf(keyAccountUsername, account)
	am.server.store.Update(func(tx *buntdb.Tx) error {
		username, _ = tx.Get(key)
		return nil
	})
	return
}
