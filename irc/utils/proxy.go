...
func readRawProxyLine(conn net.Conn, deadline time.Duration) (result []byte, err error) {
	...
	case 'P':
		...
		return readRawProxyLineV1(conn, oneByte)
	case '\r':
		// PROXY v2: starts with "\r\n\r\n"
		return readRawProxyLineV2(conn)
	default:
		return nil, ErrBadProxyLine
	}
}

func readRawProxyLineV1(conn net.Conn, oneByte []byte) (result []byte, err error) {
	var buf [maxProxyLineLen]byte // XXX: this actually escapes
	buf[0] = 'P'
	i := 1
...
func readRawProxyLineV2(conn net.Conn) (result []byte, err error) {
	// note that we have already read the first \r. finish reading the 16-byte header:
	// allocate the buffer on the heap because anything passed to (Conn).Read
	// will necessarily escape to the heap :'-(
	buf := make([]byte, 16+36)
	buf[0] = '\r'
	_, err = io.ReadFull(conn, buf[1:16])
	if err != nil {
		return
	}
