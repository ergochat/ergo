...
func serviceHelpHandler(service *ircService, server *Server, client *Client, params []string, rb *ResponseBuffer) {
	...
	if len(params) == 0 {
		...
		helpBannerLines = append(helpBannerLines, []string{
			"",
			fmt.Sprintf(client.t("To see in-depth help for a specific command, try:"), service.Name),
			ircfmt.Unescape(fmt.Sprintf("    $b/msg %s HELP <command>$b", service.Name)),
			"",
...
func makeServiceHelpTextGenerator(cmd string, banner string) func(*Client) string {
	return func(client *Client) string {
		lines := []string{
			fmt.Sprintf(client.t("%s <subcommand> [params]"), cmd),
			"",
			client.t(banner), // may contain newlines, that's fine
			"",
		}
		lines = append(lines, fmt.Sprintf(client.t("For more details, try /%s HELP"), cmd))
